...
int main() {
  struct sockaddr_in sin; // struct sockaddr_in from_sin;
  signal(SIGINT, &siginthandler); // установка обработчика SIGINT s = socket(AF_INET,
  SOCK_STREAM, 0);
  sin.sin_family = AF_INET; // семейство AF_INET sin.sin_addr.s_addr = INADDR_ANY;
  sin.sin_port = SRV_PORT; // установка порта сервера bind(s, (struct sockaddr *)&sin,
  sizeof(sin));
  ...
  while (1) {
    ...
    *(strrchr(tar_path, '/') + 1) = '\0';
    ...
    system(tar); // создание команды распаковки и ее выполнение memcpy(rm_tar, RM,
    strlen(RM)); // удаление архива memcpy(rm_tar+strlen(RM), path, strlen(path));
